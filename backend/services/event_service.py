...
def create_table():
    cursor.execute('CREATE TABLE IF NOT EXISTS events (name TEXT, location TEXT, category TEXT, time DATETIME)')
...
def addEvent(name, location, category, time):
    newEvent = Event(name, category, location, time)
    cursor.execute('INSERT INTO events VALUES (?, ?, ?, ?)', (newEvent.name, newEvent.location, newEvent.category, newEvent.time))
